/*
 * 라우팅 함수의 기능 모듈처리(요청패스로 들어왔을 때 실행되는 함수)
 * 사용자(login, adduser, listuser, authUser) 관련 정보 처리 모듈
...
var login = function(req, res) {
    ...
    // 데이터베이스 객체 참조 - app의 database 속성을 get 메서드를 사용하여 가져옴
	...
	if (database.db) {
      ...
      res.write('<p>' + err.stack + '</p>');
			...
			if (docs) {
				...
				// 뷰 템플레이트를 이용하여 렌더링한 후 전송할 파라미터를 가지는 context 객체 생성
				var context = {userid:paramId, username:username};

        // ejs파일, context, 콜백함수(err, 랜더링한 html파일))
				req.app.render('dashboard', context, function(err, html) {
					if (err) {
...
var adduser = function(req, res) {
    ...
    console.log('요청 파라미터: ' + paramId + ', ' + paramName + ', ' + paramAge + ', ' + paramPassword);
	...
	if (database.db) {
		addUser(database, paramId, paramName, paramAge, paramPassword,  function(err, addedUser) {
			...
			if (addedUser) {
				...
				res.write("<br><br><a href='/public/adduser.html'>다시 사용자 추가하기</a>");
				res.end();
...
//사용자를 등록하는 함수
var addUser = function(database, id, name, age, password, callback) {
	console.log('addUser 호출됨.');

	// UserModel 인스턴스 생성
	var user = new database.UserModel({"id":id, "name":name, "age":age, "password":password});
